...
/**
 * Update WordPress plugin from GitHub Public Repository.
 */
class GhPluginUpdater
{
    ...
    private $cache_time = 3600; // Время кэширования данных (1 час)
    ...
    public function modify_transient(object $transient): object
    {
        ...
        // Если нет информации о релизах, не обновляем
        if (empty($this->github_response)) {
            error_log("GitHub: No releases found.");
            return $transient;
        }

        // Сравниваем текущую и новую версию плагина
        if (version_compare($this->github_response['tag_name'] ?? '0.0.0', $transient->checked[$this->basename], 'gt')) {
            $plugin = [
    ...
    public function plugin_popup(bool $result, string $action, object $args)
    {
        ...
        if ($args->slug == current(explode('/', $this->basename))) {
            ...
            // Если нет данных о релизах, показываем сообщение
            if (empty($this->github_response)) {
                return (object) [
    ...
    public function after_install(bool $response, array $hook_extra, array $result): bool
    {
        ...
        // Отладка структуры архива
        error_log('Destination folder: ' . print_r($result['destination'], true));
    ...
    private function get_repository_info(): void
    {
        ...
        $cache_key = 'gh_plugin_info_' . $this->basename;
        $this->github_response = get_transient($cache_key);

        if (false === $this->github_response) {
            // Запрос без токена для открытого репозитория
            $args = [
                'method' => 'GET',
                'timeout' => 5,
                'redirection' => 5,
            ...
            // Если данные о релизах присутствуют
            if (is_array($response) && isset($response[0])) {
                $this->github_response = $response[0];
            } else {
                $this->github_response = []; // Если релизов нет
            }

            // Сохраняем данные в кэш
            set_transient($cache_key, $this->github_response, $this->cache_time);

            // Отладим данные, которые мы получаем из GitHub
            error_log('GitHub Response: ' . print_r($this->github_response, true));
        }
    }
